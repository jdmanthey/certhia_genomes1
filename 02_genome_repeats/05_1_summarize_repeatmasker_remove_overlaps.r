...
genome_index <- read.table("06_certhia_reordered.fasta.fai", sep="\t", stringsAsFactors=F)
genome_index <- genome_index[,1:2]
options(scipen=999)

# read repeatmasker file
x <- read.table("06_certhia_reordered.fasta.out", sep="", stringsAsFactors=F, fill=T)

# start output
output_name <- "06_certhia_reordered.fasta.no_dups.out"
